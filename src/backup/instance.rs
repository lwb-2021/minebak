...
impl MinecraftInstanceRoot {
    pub fn new(name: String, path: PathBuf, multimc: bool) -> Result<Self>{
        Ok(Self { 
            name, path: path.clone(), multimc, 
            instances: if multimc { MinecraftInstance::search_multimc(path)? } else {todo!()} 
        })
    }
    ...
    pub fn search_multimc(path: PathBuf) -> Result<Vec<Self>>{
        ...
        for entry in fs::read_dir(path)? {
            ...
            result.push(Self { 
                ...
                saves: MinecraftSave::search_instance(child)?
            });
        }
        Ok(result)
    }
